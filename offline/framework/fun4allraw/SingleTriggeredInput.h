...
class SingleTriggeredInput : public Fun4AllBase, public InputFileHandler
{
 public:
  ...
  virtual bool CheckPoolAlignment(const std::array<uint64_t, pooldepth>& sebdiff, const std::array<uint64_t, pooldepth>& gl1diff, std::vector<int>& bad_indices, int& shift, bool& CurrentPoolLastDiffBad, bool PrevPoolLastDiffBad);
